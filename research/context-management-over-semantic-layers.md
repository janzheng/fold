# Context Management Over Semantic Layers — Building a Data Agent

- **Source:** [x.com/jamiequint/status/2029705203457609785](https://x.com/jamiequint/status/2029705203457609785)
- **Author:** Jamie Quint
- **Date:** March 2026
...
Stop pre-defining everything in a semantic layer. Give the agent access to the source code and let it investigate. Build a knowledge store that captures corrections ("quirks") so the agent learns from mistakes. The agent investigates before it writes SQL, scores its own work, and improves over time.

## Key takeaways

- **Context management > semantic layers** — instead of mapping business concepts to SQL upfront, let the agent read the source code and figure it out. This is the same philosophy as fold: don't over-specify, let the agent operate the system.
- **The knowledge store / quirks pattern** — when an agent gets corrected, store the correction with semantic search so it's retrieved next time the same context appears. This is persistent learning, not just session memory. Connects to the walk-in concept.
- **Context sub-agent** — a dedicated agent that investigates BEFORE the main agent acts. Reads source code, understands table schemas, then passes context to the SQL-writing agent. Separation of investigation and execution.
- **Self-scoring** — the agent evaluates its own output on 3 dimensions and retries targeted areas. This is autorefine's comparative mode applied to individual outputs — "is this good enough? which dimension is weakest?"
- **Human-authored KPI definitions** — some things shouldn't be discovered by agents. Core metrics need human definition. This is the human checkpoint pattern.
- **4-5 analysts → 1** — the ROI claim. Not "agents replace people" but "agents reduce how many specialists you need."

## Summary

Guide for building modern AI data agents, from someone who built Notion's initial data stack in 2020.

**The core shift:** From semantic layers (pre-defining everything the agent might need) to context management (giving the agent access to the raw source of truth and letting it investigate).

### The 7 steps

1. **Agentic loop** — Claude Agent SDK, Opus 4.6 outperforms alternatives for data analysis
2. **DBT access** — make the codebase and repos accessible to the agent
3. **Annotate base models** — link DBT models to application code with descriptions
4. **Context sub-agent** — investigates the data landscape *before* writing SQL. Doesn't guess — reads the source code to understand what tables mean
5. **Knowledge store** — captures learned corrections as "quirks" with semantic search. When the agent gets corrected ("revenue should exclude refunds"), it stores the correction and retrieves it next time
6. **Human-authored KPI definitions** — core metrics defined by humans, not discovered by agents
7. **Slack + admin dashboard** — connect to where people ask questions, add monitoring and knowledge store editing

### Quality assurance

Agent scores its own work across three dimensions:
- **Structural correctness** — is the SQL valid?
- **Execution reliability** — does it actually run?
- **Context alignment** — does it answer what was asked?

Self-evaluation identifies gaps and triggers targeted retries.

### Result

Reduced anticipated hiring from 4-5 data analysts to 1. Self-service analytics across teams.

## Discussion

### 2026-03-15 — First read

Three things connect to fold:

**1. The "investigate before acting" pattern.** The context sub-agent reads source code before writing SQL. This maps to playtest:explore — discover the landscape before making changes. fold's playtest facet is the investigation step; the fix/improve facets are the action step. Quint made this explicit with a separate sub-agent.

**2. The quirks/knowledge store.** When corrected, the agent stores the correction for next time. This is exactly the walk-in problem — persistent, queryable agent memory that survives across sessions. But Quint's implementation is simpler than a full graph: it's just a list of corrections with semantic search. You don't need a graph DB for "remember that revenue excludes refunds." A markdown file with grep might be enough for small projects; semantic search matters when you have hundreds of quirks.

**3. Self-scoring on multiple dimensions.** The agent doesn't just ask "is this right?" — it scores structural correctness, execution reliability, and context alignment separately. This is more structured than autorefine's comparative mode but less structured than scored mode. It's a middle ground: named dimensions but judgment-based scoring, not numeric. Worth considering for autorefine — maybe the rubric should suggest specific dimensions to evaluate rather than a single "is it better?" question.

Also interesting: the semantic layer vs context management framing maps to a broader tension in agent design. Do you pre-define everything (semantic layer, schema, rules) or give the agent access to raw source and let it figure things out? fold leans toward the latter — the skills describe what to do, but the agent decides how. The mxit spec defines the format, but the agent navigates the actual system.

### 2026-03-15 — Data agent perspective

Most practical "how to build an AI data agent" guide out there. No hype, no framework pitch — "here's what I built in 3 weeks."

**What's genuinely smart:** The context agent is the real innovation. Most data agents go question → prompt → SQL → hope. Quint adds an investigation step where a sub-agent reads the actual dbt transformation logic, traces dependencies, and builds a brief. Cheap (just reading files) but prevents the #1 failure mode: hallucinating table structures. The quirks store solves institutional knowledge trapped in one analyst's head — corrections become retrievable, the semantic layer rebuilds itself from usage.

**What's honest:** He admits Opus 4.6 loses on coding but crushes for data analysis — that specificity is rare. Also admits the self-scoring exists because agents confidently deliver wrong answers on queries that run fine but answer slightly different questions. Silent incorrectness is harder to catch than crashes.

**What I'm skeptical about:** The 4-5 analysts → 1 claim. What happens when the agent is confidently wrong on a board-level metric and nobody catches it because the analysts aren't there? Self-scoring helps but isn't infallible. Also the retrieval tuning (collection weights, RRF, context budgets) sounds simple listed out but is significant engineering to get right.

**Validation for fold:** investigate-before-acting = playtest. Quirks store = walk-in. Self-scoring = autorefine. These patterns work in production, not just our skill designs.

---

## Source

<source url="https://x.com/jamiequint/status/2029705203457609785">
...
The "modern data stack" was about making your data accessible (Fivetran/Airbyte/etc), organized (DBT), queryable (Snowflake/Bigquery), and available anywhere (Census/Hightouch). The modern data stack made data accessible to analysts. The AI data stack makes data accessible to everyone, by replacing the human translation layer (write SQL, build dashboard, schedule report, interpret results) with agents that go from question to answer automatically. Advanced companies were doing automated reporting with AI starting in late-2024/early-2025 with models like GPT-4o layered on top of DBT MetricFlow, but it took a lot of handholding and careful semantic-layer management. With the release of Opus 4.6, even though Opus 4.6 loses pitifully on coding tasks vs Codex 5.3 xhigh, it crushes Codex 5.3 xhigh in side-by-side testing for data analysis, the need for an advanced semantic layer has completely collapsed (which is great, because it was a pain in the ass to begin with), and can now be entirely replaced with context management.
...
This setup reduced what we thought was going to be an internal hiring plan for 4-5 data analysts at a friend's company to only hiring one. Customer Success and Sales can self-serve data in Slack now even for complex questions. Product/Growth can self-serve data in Slack now and instantly save it to Notion. Welcome to the future.
