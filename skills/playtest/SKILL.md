---
name: playtest
description: Write agentic playtests — an LLM agent plays with your system like a creative, distracted, clever human would. Finds loopholes, boundary cases, design bugs, surprising usage patterns, and "huh" moments that test suites can't catch. Use when the user says "playtest this", "try to break it", "what's weird about this", or wants discovery beyond what scripts can test.
---
...
**Directed** — specific goals, explicit pass criteria. Use `/playtest:run`.

**Exploratory** — open-ended goals, agent decides what to look for. Use `/playtest:explore`.
...
- `/playtest:explore` — Run exploratory playtests
- `/playtest:improve` — Analyze → fix pipeline (findings become fixes)
