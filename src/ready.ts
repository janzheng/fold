...
function collectReady(
  ...
  for (const task of tasks) {
    // A task is blocked if its parent is blocked OR it has its own blocked-by
    const selfBlocked = isBlockedBy(task, openTags);
    const blocked = parentBlocked || selfBlocked;

    if (!blocked && isReady(task, openTags)) {
      ready.push(task);
    }
    ...
    if (task.children.length > 0) {
      collectReady(task.children, openTags, ready, blocked);
    }
  }
}

/** Check if a task has blocked-by tags that resolve to open tags */
function isBlockedBy(task: Task, openTags: Set<string>): boolean {
