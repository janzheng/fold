...
fold started as three separate ideas that kept bumping into each other.

We were building [mxit](https://github.com/jotaen/xit) — a markdown-native task format for AI agents. Think `- [ ] do the thing` but with statuses, tags, due dates, multi-agent claiming, crash recovery, and a proper spec. It worked. Agents could read a `TASKS.md`, pick up work, mark things done, and resume across sessions.

But we kept asking: how do you know what to put in the task list?
...
We wondered: what if the player was an LLM agent?

So we built **playtest** — a skill that tells an agent to use a system like a creative, distracted, clever human would. Not QA verification ("does the health endpoint return 200?") but discovery ("what breaks if you do something dumb? what happens at the edges? what would a clever adversary try?").

The first time we ran it on mxit itself, the agent found 22 issues in one session. A critical bug where `blocked-by` didn't cascade to child tasks. Missing CLI commands. Raw stack traces on bad input. No way to answer "what's the status of things?" — the most basic project tracking question.

None of these would have been caught by unit tests. They emerged from a real agent using the real system.

## The autorefine loop

Then we asked: ok, we found the issues. How do we systematically make things better?

We'd been studying Karpathy's [autoresearch](https://github.com/karpathy/autoresearch) — a setup where an AI agent modifies training code, runs a 5-minute experiment, checks if the metric improved, keeps or discards, and loops forever. You go to sleep and wake up to a better model. Brilliant, but it only works because `val_bpb` (the quality metric) is a single number. Lower = better.

For code quality, docs, skills, UX — there is no single number. "Better" is subjective. So we built **autorefine** with two grading modes:
...
The key innovation is the **rubric negotiation**. Before any changes happen, the agent interrogates the user: What does "better" mean to you? What doesn't matter? How would you know if it got worse? Can we measure it, or do we taste it? The user must approve the rubric before the loop starts. A wrong rubric means every iteration is wasted.

Then we autorefined autorefine. The conversation itself was iteration 1 — we added the comparative mode because the scored mode felt like false precision for subjective quality. It was better. We kept it.

## The triangle

At some point we noticed: these three tools keep pointing at each other.

- Playtest finds issues → they become mxit tasks
- mxit tracks what needs doing → autorefine works through the list
- autorefine improves things → playtest checks if they're actually better
- Repeat
...
Each is useful alone. Together they form a loop — a fold. Like folding dough: iterative, each pass makes it stronger, you never fold once.

The name stuck because it works as a verb. "Fold this project." Everyone knows what you mean: find what's wrong, track it, improve it, check again.

## What actually happened

We ran the full fold on mxit in one afternoon:
...
4. Used that feedback to autorefine the playtest skill
...
**Duration annotations are performative for agents.** This came from actual playtest feedback. Agents don't need to time themselves with "— 45s" — that's useful for humans reviewing results but feels fake when the agent writes it. We made it optional.

**The fold folds itself.** When the tools you're building are the tools you use to build them, every session produces two things: the work you intended, and feedback on the tools. This is the most productive development loop we've experienced.
